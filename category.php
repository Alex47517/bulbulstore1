...
$cat = R::findOne('categorys', 'url_name = ?', array($_GET['type']));
if (!$cat) {
	header('Location: /');
}
?>
...
<section id="category">
    <div class="container">
        <div class="row py-4">
            <div class="col-12">
                <a href="/" class="breadcrumb-main-h3">Главная > </a><a href="category.php?type=<?php echo $_GET['type']; ?>" class="breadcrumb-h3"><?php echo $cat['name'].''; ?></a>
            </div>
        </div>
    </div>
   ...
   <div class="row">
        <div class="row pb-3 pb-md-5 order-2">
            ...
            $max = 0;
            $min = 0;
            	$product = array_values(R::getAll('SELECT * FROM newproducts WHERE category = '.$cat['id'].''));
            	for ($i=0; $product[$i]; $i++) { 
...
//</div>';
                ...
                <div class="category-card h-100">
                    '.$action.'
                    <a href="'.$href.'"><div class="mb-4 category-img"><img class="mw-100" src="products_img/'.$product[$i]['img'].'?v=1"></div></a>

                    <div class="form-row form-row__custom">
            	...
            	if ($i == 0) {
                $sort = null;
            		echo "<h3 class=\"pl-3\">В этой категории нет товаров</h3>";
            	}
            ?>
        </div>  
        <?php echo $sort; ?>
