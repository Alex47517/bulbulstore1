...
$product = array_values(R::getAll("SELECT * FROM `newproducts` WHERE `ad_name` LIKE '%".$_GET['search']."%' AND `disabled` = 0"));
if (!$product[1] && $product[0]) {
header('Location: /product.php?name='.$product[0]['url_name'].'');
}
?>
<!DOCTYPE HTML>
<html>
<head>
    <?php $title = 'Результаты поиска | BulBul'; require 'parts/head.php'; ?>
    <!-- Facebook Pixel Code -->
<script>
...
<?php $search_head = $_GET['search']; require 'parts/second_header.php'; ?>
<section id="category">
    <div class="container">
        <div class="row py-4">
            <div class="col-12">
                <a href="/" class="breadcrumb-main-h3">Главная > </a><a href="search.php" class="breadcrumb-h3">Поиск > </a>
            </div>
        </div>
    </div>
    <div class="category-block">
    <div class="container">
        <h3 class="category-h3 med-pad">Результаты поиска по запросу "<?php echo $_GET['search']; ?>":</h3>
        <div class="container">
   <div class="row">
...
<button onclick="filterElements()" class="btn col-2 offset-1">ОК</button>
                  ...
                  $href = 'product.php?name='.$product[$i]['url_name'];
            			echo '<div class="col-12 col-md-6 col-lg-3 pb-3 pb-md-0 mb-5 box" data-price="'.$product[$i]['sum'].'">
                        ...
                        <div class="form-row">
                          ...
                          <div class="form-row">
                              <div class="form-group col-6 col-md-6">
                                ...
                                <div class="form-group col-6 col-md-6"><button onclick="addtoCart('.$product[$i]['id'].')" class="btn cart-btn">В корзину</button>
            	...
            	if ($i == 0) {
                $sort = null;
            		echo "<h3 class=\"pl-3\">Ничего не найдено :(</h3>";
            	}
            ?>
        </div>  
        <?php //echo $sort; ?>
